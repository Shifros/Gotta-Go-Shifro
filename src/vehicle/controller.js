...
import { closestOnRoad, roadPointAhead, roadSamples, roadLength, roadSurfaceAt, roadFrameAt, ROAD_LIFT, ROAD_CROWN } from '../world/road.js';
...
const POWER = [[0, 2.2], [16.67, 3.0], [38.89, 2.95], [55.56, 2.95], [66.67, 3.9], [94.44, 6.8]];
function powerAt(v) {
  v = Math.abs(v);
  if (v <= POWER[0][0]) return POWER[0][1];
  for (let i = 0; i < POWER.length - 1; i++) {
    const [x0, y0] = POWER[i], [x1, y1] = POWER[i + 1];
...
export class CarController {
  ...
  ensureAudio() {
    ...
    try {
      ...
      const o2 = ctx.createOscillator(); o2.type = 'square'; o2.frequency.value = 35;
      const g2 = ctx.createGain(); g2.gain.value = 0.4;
      o1.connect(filt); o2.connect(g2); g2.connect(filt); filt.connect(engGain); engGain.connect(master);
      o1.start(); o2.start();
      // wind
      ...
      noise.start();
      this.audio = { ctx, master, engGain, filt, o1, o2, wGain, bp };
    } catch { /* no audio */ }
  }
  toggleMute() {
  ...
  toggleAutosteer() { this.autosteer = !this.autosteer; return this.autosteer; }
  ...
  update(dt, elapsed) {
    ...
    const offroad = closest.dist > ROAD_WIDTH * 0.62;
    ...
    if (this.autosteer) {
      ...
      const ahead = roadPointAhead(closest.index, look).point;
      ...
      const cap = 16.7; // 60 km/h cruise like the reference HUD
      if (this.speed < cap - 0.5) throttle = Math.max(throttle, err > 0.6 ? 0.3 : 0.66);
      else if (this.speed > cap + 1.2) { brake = Math.max(brake, 0.5); throttle = 0; }
      else throttle = Math.max(throttle, 0.17); // feather the 60 km/h cruise
      if (inp.down) { throttle = 0; } // driver brake overrides cruise
    ...
    let accel = throttle * powerAt(this.speed) * (this.boosting ? 1 : 0.62);
    if (brake > 0) {
    ...
    // soft ceiling: coast down past the cap instead of hitting a wall
    if (this.speed > cap) {
      this.speed = Math.max(cap, this.speed - Math.max(0, this.speed - cap) * 1.6 * dt - 1.2 * dt);
    }
    ...
    if (Math.abs(this.speed) > 0.15) {
      const rawYaw = (this.speed / 2.84) * Math.tan(this.steerAngle);
    ...
    // ----- collision: stone wall strip -----
    if (closest.index >= 78 && closest.index <= 332) {
      const s = closest.point;
      const lnx = -s.tz, lnz = s.tx;
      const lat = (this.pos.x - s.x) * lnx + (this.pos.z - s.z) * lnz;
      const wallOff = ROAD_WIDTH / 2 + 1.7;
      if (Math.abs(lat - wallOff) < 1.15) {
        const side = lat > wallOff ? 1 : -1;
        this.pos.x += lnx * side * (1.15 - Math.abs(lat - wallOff));
        this.pos.z += lnz * side * (1.15 - Math.abs(lat - wallOff));
        if (Math.abs(this.speed) > 6) this.crashed = Math.max(this.crashed, 0.7);
        this.speed *= 0.72;
      }
    }
    ...
    // ----- ground: ride the finished tarmac on-road, dirt off-road -----
    const terrH = getHeight(this.pos.x, this.pos.z);
    let groundY = terrH;
    const rs = roadSurfaceAt(this.pos.x, this.pos.z);
    if (rs) {
      const a = Math.abs(rs.lat);
      if (a < ROAD_WIDTH / 2 + 1.6) {
        const b = 1 - smoothstep(ROAD_WIDTH / 2 - 0.5, ROAD_WIDTH / 2 + 1.6, a);
  ...
  updateAudio(offroad) {
    ...
    const a = this.audio;
    const spd = Math.abs(this.speed);
    const rpm = 55 + spd * 4.4 + this.throttleVis * 26;
    a.o1.frequency.setTargetAtTime(rpm, a.ctx.currentTime, 0.06);
    a.o2.frequency.setTargetAtTime(rpm * 0.5 + 3, a.ctx.currentTime, 0.06);
    a.filt.frequency.setTargetAtTime(320 + spd * 46 + this.throttleVis * 500, a.ctx.currentTime, 0.1);
    a.engGain.gain.setTargetAtTime(0.035 + this.throttleVis * 0.05 + spd * 0.0006, a.ctx.currentTime, 0.1);
    a.wGain.gain.setTargetAtTime(Math.pow(spd / 60, 2) * 0.22 + (offroad ? 0.02 : 0), a.ctx.currentTime, 0.2);
  }
}
